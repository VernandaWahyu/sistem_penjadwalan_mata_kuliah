...
<head>
    ...
    <title>Update Data Pengampu - UMPO</title>
    <!-- General CSS Files -->
                    ...
                    <ul class="sidebar-menu">
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a class="nav-link" href="<?=base_url('admin/penjadwalan_round_robin')?>">Jadwal Penjadwalan Round Robin</a>
                                </li>
                            </ul>
                        </li>
                ...
                <section class="section">
                    <div class="">
                        <div class="card" style="width:100%;">
                            <div class="card-body">
                                <h2 class="card-title" style="color: black;">Update Data pengampu</h2>
                                <hr>
                                <p class="card-text"> Update data siswa meliputi Nama pengampu, Jumlah Siswa dan Semester.
...
<form method="POST" action="<?= base_url('admin/pengampu_edit') ?>">
    <div class="form-group">
        <label for="id_pengampu">ID Pengampu</label>
        <input readonly id="id_pengampu" type="text" class="form-control" value="<?= $pengampu->id_pengampu ?>" name="id_pengampu">
        <?= form_error('id_pengampu', '<small class="text-danger">', '</small>'); ?>
    </div>

    <div class="form-group">
        <label for="id_dosen">NIK Dosen</label>
        <select id="id_dosen" class="form-control" name="id_dosen">
            <option value="">Pilih Dosen</option>
            <?php foreach ($dosen as $d): ?>
                <option value="<?= $d->id_dosen; ?>" <?= $d->id_dosen == $pengampu->id_dosen ? 'selected' : ''; ?>>
                    <?= $d->nik; ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-group">
        ...
        <select id="id_matkul" class="form-control" name="id_matkul">
            <option value="">Pilih Mata Kuliah</option>
            <?php foreach ($mata_kuliah as $mk): ?>
                <option value="<?= $mk->id_matkul; ?>" <?= $mk->id_matkul == $pengampu->id_matkul ? 'selected' : ''; ?>>
                    <?= $mk->nama_matakuliah; ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-group">
        ...
        <select id="id_kelas" class="form-control" name="id_kelas">
            ...
            <?php foreach ($kelas as $k): ?>
                <option value="<?= $k->id_kelas; ?>" <?= $k->id_kelas == $pengampu->id_kelas ? 'selected' : ''; ?>>
                    <?= $k->nama_kelas; ?>
                </option>
            <?php endforeach; ?>
        </select>
    ...
    <button type="submit" class="btn btn-primary">Update Pengampu</button>
</form>
